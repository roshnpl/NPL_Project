...
class NPI():
    ...
    def npi_core(self):
        ...
        for i in range(self.npi_core_layers):
            out=tf.keras.layers.LSTM(units=self.npi_core_dim,return_sequences=True,stateful=True,return_state=True)(c)
            c=out[0]
    ...
    def argument_net(self):
        ...
        for i in range(self.num_args):
            arg=tf.keras.layers.Dense(units=self.arg_depth,kernel_regularizer=tf.keras.regularizers.l2,name='Argument_{}'.format(str(i)))(self.h)
